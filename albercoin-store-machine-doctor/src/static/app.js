...
function renderStatus(state){
  ...
  renderThrottling(sample);
  renderFinal(state);
}

function renderThrottling(sample){
  const root=document.getElementById('throttling-info');
  const throttling=sample?.throttling;
  root.className='thermal-status';
  if(!throttling){root.textContent=t('throttlingWaiting');return}
  if(throttling.confirmed){root.textContent=t('throttlingConfirmed');root.classList.add('bad');return}
  if(throttling.available){root.textContent=t('throttlingNotDetected');root.classList.add('ok');return}
  root.textContent=t('throttlingUnavailable');root.classList.add('warn');
}
