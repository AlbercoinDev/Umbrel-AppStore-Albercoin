...
```
docker-compose.yml   # 4 services: web, init, server, tor
...
settings/settings.html  # Settings page (served by web container as /settings.html)
icon.png
```
...
Available at `/settings.html` on the app's web UI. Shows current config values read from `config.json`. To change values: set env vars + restart app.
