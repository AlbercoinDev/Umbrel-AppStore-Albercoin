...
| Service | Image | Role |
|---------|-------|------|
| `web` | `ghcr.io/4rkad/frigate-umbrel-web` | Nginx UI, serves settings page, mounts `config.json` + `settings.html` |
| `api` | `alpine:3.20` | HTTP API (socat) on 8081, handles POST /api/save → writes `/data/config.env`, re-runs init.sh, restarts server via Docker socket |
...
Available at `/settings.html` on the app's web UI. Shows current config values read from `config.json` in editable form fields. Click **Save &amp; Restart** to POST JSON to `/api/save` → writes `/data/config.env`, runs `init.sh` to regenerate `config.toml` + `config.json`, then restarts the server container via the Docker socket API. The page reloads automatically after 3s.
...
- **This wrapper**: https://github.com/semillabitcoin/frigate-umbrel
